    ...
    GenericDaoPostgres<SubjectPartialEntity, int>(context), ISubjectPartialDao
{
    ...
    {
        return await entities.AsNoTracking()
            .Where(e => e.partialId == partialId)
            .Where(sub => context.Set<EnrollmentEntity>()
                .Any(e => e.enrollmentId == sub.enrollmentId && e.degreeId == degreeId))
            .Include(e => e.gradeList)
            .ThenInclude(e => e.student)
            .ToListAsync();
    }

    public async Task<List<SubjectPartialEntity>> getListByTeacherAndPartial(string teacherId, int partialId)
    {
        return await entities.AsNoTracking()
            .Where(e => e.partialId == partialId && e.partialId == partialId)
            .Include(e => e.gradeList)
            .ThenInclude(e => e.student)
            .ToListAsync();
    }
